...
import AirTravel from "../models/airTravel.js";

const index = async (req, res) => {
    const query = req.query;

    let allData = await AirTravel.find(query);
...
const add = async (req, res) => {
    try {
        const result = new AirTravel(req.body);
        await result.save();
...
const addMany = async (req, res) => {
    try {
        const data = req.body;
        const result = await AirTravel.insertMany(data);
...
const edit = async (req, res) => {
    try {
        ...
        let result = await AirTravel.findByIdAndUpdate(
            { _id: req.params.id },
            { $set: req.body },
            { new: true }
        );
...
const deleteData = async (req, res) => {
    try {
        let result = await AirTravel.findOneAndDelete({ _id: req.params.id })
...
const deleteMany = async (req, res) => {
    try {
        ...
        const result = await AirTravel.deleteMany({ _id: { $in: ids } });
