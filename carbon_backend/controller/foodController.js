...
const index = async (req, res) => {
    ...
    let allData = await Food.find(query);
...
const add = async (req, res) => {
    try {
        const result = new Food(req.body);
        await result.save();
...
const addMany = async (req, res) => {
    try {
        const data = req.body;
        const result = await Food.insertMany(data);
...
const deleteData = async (req, res) => {
    try {
        let result = await Food.findOneAndDelete({ _id: req.params.id })
