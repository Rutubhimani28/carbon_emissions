...
const addEmail = async (req, res) => {
    try {
        // const { subject, receiver, sender, data, templateName, activityName, name, totalTonCo2, eveydolarCo2, resultTableData } = req.body;
        const { subject, receiver, sender, data, emailBodyTemplateName, attachmentTemplateName, attachmentPdfName, activityName, name, totalTonCo2, eveydolarCo2, resultTableData, chatSuggestion, attachmentTemplateNameOne, attachmentPdfNameOne, attachmentTemplateNameTwo, attachmentPdfNameTwo, attachmentTemplateNameThree, attachmentPdfNameThree, attachmentTemplateNameFour, attachmentPdfNameFour, dataOne, dataTwo, dataThree, dataFour, totalTonCo2One, totalTonCo2Two, totalTonCo2Three, totalTonCo2Four, eveydolarCo2One, eveydolarCo2Two, eveydolarCo2Three, eveydolarCo2Four, resultTableDataOne, resultTableDataTwo, resultTableDataThree, resultTableDataFour } = req.body;
...
const addEmailForGraphs = async (req, res) => {
    try {
        const { allEventsEmissions, receiver, attachmentPdfName, attachmentTemplateName, subject } = req.body;
...
const addEmailForTwoEvents = async (req, res) => {
    try {
        ...
        if (!eventsData || eventsData?.length < 1) {
            return res.status(400).json({ success: false, message: 'Needed two events data.' });
        }
...
export default { addEmail, addEmailForGraphs, addEmailForTwoEvents };