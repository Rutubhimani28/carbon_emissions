...
const index = async (req, res) => {
    ...
    let allData = await production.find(query)
...
const add = async (req, res) => {
    try {
        const result = new production(req.body);
        await result.save();
...
const addMany = async (req, res) => {
    try {
        const data = req.body;
        const result = await production.insertMany(data);
...
const edit = async (req, res) => {
    try {
        ...
        let result = await production.findByIdAndUpdate(
            { _id: req.params.id },
            { $set: req.body }
        );
...
const deleteData = async (req, res) => {
    try {
        let result = await production.findOneAndDelete({ _id: req.params.id })
