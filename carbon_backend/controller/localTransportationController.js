...
import LocalTransport from "../models/localTransportation.js";

const index = async (req, res) => {
    const query = req.query;

    let allData = await LocalTransport.find(query);
...
const add = async (req, res) => {
    try {
        const result = new LocalTransport(req.body);
        await result.save();
...
const addMany = async (req, res) => {
    try {
        const data = req.body;
        const result = await LocalTransport.insertMany(data);
...
const edit = async (req, res) => {
    try {
        let result = await LocalTransport.findByIdAndUpdate(
            { _id: req.params.id },
            { $set: req.body },
            { new: true }
        );
...
const deleteData = async (req, res) => {
    try {
        let result = await LocalTransport.findOneAndDelete({ _id: req.params.id })
...
const deleteMany = async (req, res) => {
    try {
        ...
        const result = await LocalTransport.deleteMany({ _id: { $in: ids } });
