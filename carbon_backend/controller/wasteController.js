...
const index = async (req, res) => {
    ...
    let allData = await Waste.find(query);
...
const add = async (req, res) => {
    try {
        const result = new Waste(req.body);
        await result.save();
...
const addMany = async (req, res) => {
    try {
        const data = req.body;
        const result = await Waste.insertMany(data);
...
const edit = async (req, res) => {
    try {
        let result = await Waste.findByIdAndUpdate(
            { _id: req.params.id },
            { $set: req.body },
            { new: true }
        );
...
const deleteData = async (req, res) => {
    try {
        let result = await Waste.findOneAndDelete({ _id: req.params.id })
...
const deleteMany = async (req, res) => {
    try {
        ...
        const result = await Waste.deleteMany({ _id: { $in: ids } });
