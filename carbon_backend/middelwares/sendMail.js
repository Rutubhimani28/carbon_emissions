...
export const sendMailForDateRangeEvents = async ({
  eventsData,
  startDate,
  endDate,
  ...
  try {
    ...
    // Create the data for the combined report (all events)
    const combinedData = {
      startDate,
      endDate,
      eventsData,
    ...
    // Define the template path dynamically
    const templatePath = path.join(
      __dirname,
      "email_templates",
      "date-report-template.ejs"
    );

    // Render the email template with the combined data for all events
    const renderedTemplate = await ejs.renderFile(templatePath, combinedData);

    // Define the path for the combined PDF
    const pdfFilePath = path.join(
      __dirname,
      `Carbon_Report_${startDate}_to_${endDate}.pdf`
    );

    try {
      await createPDF(renderedTemplate, pdfFilePath, logoBase64);
    } catch (pdfError) {
      console.error(`Error generating PDF for the date range:`, pdfError);
      throw pdfError;
    }

    attachmentsArray.push({
      filename: `Carbon_Report_${startDate}_to_${endDate}.pdf`,
      path: pdfFilePath,
      contentType: "application/pdf",
    });
    const htmlData = `
