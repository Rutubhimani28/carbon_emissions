...
import { fetchAirTravelData } from '../../redux/slice/airTravelSlice'
                    ...
                    <Collapse in={open} timeout='auto' unmountOnExit>
                        <Box >
                            ...
                            <Table size='small' aria-label='purchases'>
                                <TableHead>
                                    <TableRow>
                                        <TableCell className='fw-bolder text-muted fs-6 text-center'>No of Trips</TableCell>
                                ...
                                <TableBody>
                                    {row?.details?.map((historyRow) => (
                                        <TableRow key={historyRow.employee1} sx={{ borderBottom: 0 }}>
                                            <TableCell className='text-dark fw-bolder fs-6 text-center'>{historyRow.noOfTrips || '-'}</TableCell>
...
export default function AirTravel() {
    ...
    const { data, isLoading } = useSelector((state) => state?.airTravelDetails)
