...
import { fetchTransportationData } from '../../redux/slice/transportationSlice';
...
function Row(props) {
  ...
  const list1 = ['Petrol Car', 'Diesel Car', 'SUV Diesel', 'SUV Petrol', 'Camper Petrol', 'Caper Diesel'];

  const handleDelete = async (id) => {
    setIsLoading(true);
    try {
      const result = await apidelete(`api/transportation/${id}`);
              ...
              <Table size="small" aria-label="purchases">
                <TableHead>
                  <TableRow>
                    ...
                    {list1.includes(`${row?.name}`) && (
                      <TableCell className="fw-bolder text-muted fs-6 text-center">No of Passengers</TableCell>
                    )}
                ...
                <TableBody>
                  {row?.details?.map((historyRow) => (
                    <TableRow key={historyRow.employee1} sx={{ borderBottom: 0 }}>
                      <TableCell className="text-dark fw-bolder fs-6 text-center">
                        {historyRow.noOfKms || '-'}
                      </TableCell>
                      {list1.includes(`${row?.name}`) && (
                        <TableCell className="text-dark fw-bolder fs-6 text-center">
                          {historyRow.noOfPassengers || '-'}
                        </TableCell>
                      )}
                      <TableCell className="text-dark fw-bolder fs-6 text-center">
                        <span className="pe-4 text-success " style={{ cursor: 'pointer' }}>
                          <EditIcon
                            onClick={() => {
                              handleOpenAdd();
                              setType('edit');
                              setSelectedData(historyRow);
                            }}
                          />
                        </span>
