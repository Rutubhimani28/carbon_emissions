...
import { fetchEnergyData } from '../../redux/slice/energySlice'
...
function Row(props) {
    ...
    const handleDelete = async (id) => {
        setIsLoading(true)
        try {
            const result = await apidelete(`api/energy/${id}`)
                    ...
                    <Collapse in={open} timeout='auto' unmountOnExit>
                        <Box >
                            ...
                            <Table size='small' aria-label='purchases'>
                                <TableHead>
                                    <TableRow>
                                        <TableCell className='fw-bolder text-muted fs-6 text-center'>{row?.name === "Electricity" ? "Kwh" : "Gallons"}</TableCell>
                                ...
                                <TableBody>
                                    {row?.details?.map((historyRow) => (
                                        <TableRow key={historyRow.employee1} sx={{ borderBottom: 0 }}>
                                            <TableCell className='text-dark fw-bolder fs-6 text-center'>{row?.name === "Electricity" ? historyRow.kwh || '-' : historyRow.gallons || '-'}</TableCell>
...
export default function Energy() {
    ...
    const { data, isLoading } = useSelector((state) => state?.energyDetails)
